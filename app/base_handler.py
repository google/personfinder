...
class Handler(webapp.RequestHandler):
    """Base handler class for all pf handlers.  Takes care of parsing & 
    ...
    def initialize(self, *args):
        ...
        expiry_keys = utils.PERSON_EXPIRY_TEXT.keys().sort()
        self.env.expiry_options = [
            Struct(value=value, text=utils.PERSON_EXPIRY_TEXT[value])
        ...
        if self.params.key:
            if self.subdomain: 
                ...
                self.auth = model.Authorization.get(self.subdomain, 
                                                    self.params.key)
            if not self.auth:
