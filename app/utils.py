...
def get_instance_list(lang):
    ...
    for subdomain in model.Subdomain.list():
        titles = config.get_for_subdomain(subdomain, 'subdomain_titles') or ['']
...
class Main(webapp.RequestHandler):
    ...
    def dispatch_to_handler(self, name, lang):
        ...
        getattr(handler, self.request.method.lower())()  # call get() or post()
...
class Handler(webapp.RequestHandler):
    ...
    def redirect(self, url, **params):
        if re.match('^[a-z]+:', url):
            ...
            url = get_url(self.request, url, self.env.charset, **params)
    ...
    def get_start_url(self, subdomain=None):
        ...
        return get_url(self.request, '/', self.env.charset, subdomain=subdomain)
    ...
    def initialize(self, request, response, env):
        ...
        if random.random() < sample_rate:
            model.UserAgentLog(
                subdomain=env.subdomain, sample_rate=sample_rate,
                user_agent=self.request.headers.get('User-Agent'), lang=env.lang,
        ...
        if self.params.key:
            if env.subdomain: 
                ...
                self.auth = model.Authorization.get(env.subdomain, self.params.key)
            if not self.auth:
