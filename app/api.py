...
class Write(utils.Handler):
  def post(self):
    ...
    if auth:
      ...
      try:
        person_records, note_records = pfif.parse_file(self.request.body_file)
      except Exception, e:
      ...
      self.write_status('note', written, skipped, total, 'note_record_id')
  ...
  def write_status(self, type, written, skipped, total, id_field):
    """Emit the status information about the results of an attempted write."""
    skipped_records = []
    for error, record in skipped:
      skipped_records.append('      <pfif:%s>%s</pfif:%s>\n' %
                             (id_field, record.get(id_field, ''), id_field))
      skipped_records.append('      <status:error>%s</status:error>\n' % error)
