...
class Read(utils.Handler):
    ...
    def get(self):
        ...
        person = model.Person.get(
            self.repo_name, record_id, filter_expired=False)
        if not person:
            return self.error(404, 'No person record with ID %s' % record_id)
        notes = model.Note.get_by_person_record_id(self.repo_name, record_id)
...
class Write(utils.Handler):
    ...
    def post(self):
        ...
        num_people_written, people_skipped, total = importer.import_records(
            self.repo_name, source_domain, create_person, person_records)
        self.write_status(
        ...
        num_notes_written, notes_skipped, total = importer.import_records(
            self.repo_name, source_domain, create_note, note_records,
...
class Search(utils.Handler):
    ...
    def get(self):
        ...
        if self.config.external_search_backends:
            results = external_search.search(self.repo_name, query, max_results,
        ...
        if not results:
            results = indexing.search(self.repo_name, query, max_results)
...
class Subscribe(utils.Handler):
    ...
    def post(self):
        ...
        subscription = subscribe.subscribe_to(self, self.repo_name, person,
...
class Unsubscribe(utils.Handler):
    ...
    def post(self):
        ...
        subscription = model.Subscription.get(self.repo_name, self.params.id,
                                              self.params.subscribe_email)
