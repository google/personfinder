...
def send_notifications(person, note, handler, marked_dup=False):
    """Sends status updates about the person to its subscribers and to
    the subscribers of all person records marked as duplicates. If
    marked_dup is True, assume the note was generated by the marking
    of two Person records as duplicates. """
    sender = get_sender(handler)
    original_person_url = handler.get_url('/view', id=person.record_id)
    linked_person_url = None
    if marked_dup:
      linked_person_url=handler.get_url('/view', id=note.linked_person_record_id)
    person_set = person.get_linked_persons_all()
    try:
        for p in person_set:
            for sub in p.get_subscriptions():
                subscribed_to_dup = (p != person)
                if subscribed_to_dup:
