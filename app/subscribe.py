...
def subscribe_to(handler, repo_name, person, email, lang):
    """Add a subscription on a person for an e-mail address"""
    existing = model.Subscription.get(repo_name, person.record_id, email)
    ...
    if existing:
        ...
        subscription = model.Subscription.create(
            repo_name, person.record_id, email, lang)
    db.put(subscription)
...
class Subscribe(Handler):
    ...
    def post(self):
        ...
        subscription = subscribe_to(self, self.repo_name, person,
