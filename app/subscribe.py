...
def send_notifications(person, note, handler, marked_dup=False):
    ...
    duplicates will be notified. If marked_dup is True, assume the note
    was generated by the marking of two Person records as duplicates.
    """
    sender = get_sender(handler)
    original_person_url = handler.get_url('/view', id=person.record_id)
    linked_person_url = None
    if marked_dup:
      linked_person_url=handler.get_url('/view',
                                        id=note.linked_person_record_id)
