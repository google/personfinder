...
class Handler(BaseHandler):
    ...
    def get(self):
        url_link = '/create_volunteer' if self.params.role == 'volunteer' else '/create'  
        ...
        if self.params.role == 'volunteer':
            ...
            try:
                ...
                logging.error("failed to query by phone")
                results = None
 
            results_url = self.get_results_url(author_phone)
            if results:
                person = results[0]
                self.redirect('/view', id=person.record_id, query=self.params.query)
            else:
                if self.env.ui == 'small':
                    ...
                    return self.redirect('/create_volunteer', **self.params.__dict__)
        ...
        if self.params.role == 'provide':
            ...
            results = self.search(query_txt, self.params.role)
        ...
        if self.params.role == 'seek' or self.params.role == 'seek_volunteer':
