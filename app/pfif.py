...
class PfifVersion:
  ...
  def check_tag(self, (ns, local), parent=None):
    """Given a namespace-qualified tag and its parent, returns the PFIF type
    or field name if the tag is valid, or None if the tag is not recognized."""
    if ns == self.ns:
  ...
  def write_file(self, file, persons, get_notes_for_person=lambda p: []):
    ...
    records for each person, and writes PFIF to the given file object.  Each
...
def split_first_last_name(all_names):
  ...
  names = name.split()
  if len(names) > 1:
    last_name = re.search(r' (\S*(-+ | -+|-+)?\S+)\s*$', name).group(1).strip()
...
class Handler(xml.sax.handler.ContentHandler):
  ...
  def endElementNS(self, tag, qname):
    ...
    if check_pfif_tag(tag) == 'person':
      ...
      # Save all parsed notes (whether or not they are enclosed in <person>).
